package com.coresmore.xfosd.libs;

import java.util.List;
import java.util.UUID;

import com.coresmore.xfosd.b_interface.IBleConnectListener;
import com.coresmore.xfosd.b_interface.IBleDisConnectListener;
import com.coresmore.xfosd.libs.bluetooth.BleBluetooth;
import com.coresmore.xfosd.libs.bluetooth.MultipleBluetoothController;
import com.coresmore.xfosd.libs.bluetooth.SplitWriter;
import com.coresmore.xfosd.libs.callback.BleGattCallback;
import com.coresmore.xfosd.libs.callback.BleIndicateCallback;
import com.coresmore.xfosd.libs.callback.BleMtuChangedCallback;
import com.coresmore.xfosd.libs.callback.BleNotifyCallback;
import com.coresmore.xfosd.libs.callback.BleReadCallback;
import com.coresmore.xfosd.libs.callback.BleRssiCallback;
import com.coresmore.xfosd.libs.callback.BleScanAndConnectCallback;
...
import com.coresmore.xfosd.libs.exception.OtherException;
import com.coresmore.xfosd.libs.scan.BleScanRuleConfig;
import com.coresmore.xfosd.libs.scan.BleScanner;
import com.coresmore.xfosd.libs.utils.BleLog;
...
public class BleManager {
    ...
    private IBleDisConnectListener disConnectListener;
    ...
    /**
     * 将监听放在Manger里面。方便切换activity之后获取新的监听
     * */
    public BluetoothGatt connect(BleDevice bleDevice, final IBleConnectListener connectListener) {
        ...
        if (!isBlueEnable()) {
            BleLog.e("Bluetooth not enable!");
                connectListener.onConnectFail(bleDevice, new OtherException("Bluetooth not enable!"));
            return null;
        }
        ...
        if (bleDevice == null || bleDevice.getDevice() == null) {
                connectListener.onConnectFail(bleDevice, new OtherException("Not Found Device Exception Occurred!"));
        } else {
            BleBluetooth bleBluetooth = multipleBluetoothController.buildConnectingBle(bleDevice);
            boolean autoConnect = bleScanRuleConfig.isAutoConnect();
            return bleBluetooth.connect(bleDevice, autoConnect, new BleGattCallback() {
                ...
                public void onDisConnected(boolean isActiveDisConnected, BleDevice device, BluetoothGatt gatt, int status) {
                    connectListener.onDisConnected(isActiveDisConnected, device, gatt, status);
                    if (null != disConnectListener) {
                        disConnectListener.onDisConnected(isActiveDisConnected, device, gatt, status);
                    }
                }
    ...
    public BluetoothGatt connect(String mac, IBleConnectListener connectListener) {
        BluetoothDevice bluetoothDevice = getBluetoothAdapter().getRemoteDevice(mac);
        BleDevice bleDevice = new BleDevice(bluetoothDevice, 0, null, 0);
        return connect(bleDevice, connectListener);
    }
    
    public void setIBleDisConnectListener(IBleDisConnectListener disConnectListener){
